...
namespace util {
	...
	inline bool isRainmeterFormula( const std::wstring &v ) {
		...
		// Case 1: String is enclosed in parentheses (Rainmeter golden rule)
		if ( end - start + 1 >= 3 && v[start] == L'(' && v[end] == L')' )
			return true;


		// Case 2: String is not enclosed in parentheses. Check operators more carefully
		size_t opPos = v.find_first_of( L"+-*/%^", start );
		while ( opPos != std::wstring::npos ) {
			// Check the characteristic prior to the operator
			wchar_t prevChar = ( opPos > start ) ? v[opPos - 1] : L'\0';
			bool validPrev = ( prevChar == L' ' ) || isdigit( prevChar ) || prevChar == L')' || prevChar == L'#';

			// Check the next character after the operator
			wchar_t nextChar = ( opPos + 1 <= end ) ? v[opPos + 1] : L'\0';
			bool validNext = ( nextChar == L' ' ) || isdigit( nextChar ) || nextChar == L'(' || nextChar == L'#';

			// If the operator appears to be in a mathematical context, it returns true
			if ( validPrev || validNext )
				return true;

			// Looking for the next one
			opPos = v.find_first_of( L"+-*/%^", opPos + 1 );
		}
	 ...
	 * @endcode
	 */
	template <typename TRain> inline bool RmVarInt( TRain *rain, LPCWSTR var, lua_Integer &out ) {
	...
	inline std::wstring CanonicalizePath( const std::wstring &path ) {
		...
		DWORD len = GetFullPathNameW( path.c_str(), required, buffer.data(), nullptr );
